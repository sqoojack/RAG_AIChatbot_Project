...
# Upload_file.py
from utils import io, pytesseract, base64, requests, re, json, ffmpeg, tempfile, client, os, whisper
...
def load_documents_from_upload(uploaded_file, img_model):
    ...
    # 處理 mp4 (先只抽音訊, 把他轉成mp3後採用跟mp3一樣的處理方式) 要下載ffmpeg才能轉檔, 目前公司會擋所以暫時無法用mp4
        ...
        try:
            # 1. 先將 mp4 轉換為音訊檔案（WAV 格式）
            wav_bytes, _ = (ffmpeg.input("pipe:0").output("pipe:1", format="wav").run(input=content, capture_stdout=True, capture_stderr=True))
            ...
            # 捕捉錯誤並輸出
            print(f"Error processing mp4 file: {e}")
            return []
    # 測試問題: 上傳docker教學影片並問說 根據資料幫我準備Docker的ppt內容
